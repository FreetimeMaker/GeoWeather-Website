  ...
  return (
    <div className="site-shell">
      <nav className="nav" aria-label="Main navigation">
        ...
        <div className="nav-links"><a href="#features">Features</a><a href="#preview">Preview</a><a href="#open-source">Open source</a><a href="#feedback">Feedback</a></div>
      ...
      <main id="top">
        ...
        <section className="section" id="feedback"><div className="section-heading reveal"><p className="eyebrow">04 / Shape GeoWeather</p><h2>Your ideas can<br /><em>shape what comes next.</em></h2><p>Have an idea for improving GeoWeather? Share a suggestion or request a feature directly on GitHub.</p></div><div className="feature-grid"><FeedbackCard className="feature-blue" icon="✦" kicker="SUGGESTIONS" title="Share an idea." number="01" href={`${repository}/issues/new?title=%5BSuggestion%5D%20&body=%23%23%20Suggestion%0A%0ADescribe%20your%20idea%20and%20how%20it%20could%20improve%20GeoWeather.%0A%0A%23%23%20Why%20would%20this%20be%20useful%3F%0A%0A`} action="Send a suggestion">Got an idea for the app, design or weather experience? Tell us what you would change and why it would help.</FeedbackCard><FeedbackCard className="feature-yellow" icon="＋" kicker="FEATURE REQUESTS" title="Request a feature." number="02" href={`${repository}/issues/new?template=feature_request.md`} action="Request a feature">Need something GeoWeather does not support yet? Open a feature request and describe the functionality you would like to see.</FeedbackCard></div></section>

        <section className="download section" id="get-app"><div className="download-heading"><p className="eyebrow">05 / Take it with you</p><h2>A better forecast<br /><em>is one tap away.</em></h2></div><div className="download-links"><Download href={`${repository}/releases/latest`} icon={icons.github} alt="GitHub" title="GitHub Releases" detail="Direct APK downloads" /><Download href={fDroid} icon={icons.fDroid} alt="F-Droid" title="F-Droid" detail="Free and open source" /><Download href="https://apps.obtainium.imranr.dev/redirect?r=obtainium://app/%7B%22id%22%3A%22com.freetime.geoweather%22%2C%22url%22%3A%22https%3A%2F%2Fgithub.com%2FFreetimeMaker%2FGeoWeather%22%2C%22author%22%3A%22Freetime%20Maker%22%2C%22name%22%3A%22GeoWeather%22%2C%22additionalSettings%22%3A%22%7B%5C%22includePrereleases%5C%22%3Afalse%7D%22%7D" icon={icons.obtainium} alt="Obtainium" title="Obtainium" detail="Automatic updates" /></div></section>
      </main>
...
function FeedbackCard({ className, icon, kicker, title, number, href, action, children }) { return <article className={`feature-card ${className} reveal`}><div className="feature-icon">{icon}</div><div><p className="card-kicker">{kicker}</p><h3>{title}</h3><p>{children}</p><a className="text-link" href={href} target="_blank" rel="noreferrer">{action} <span>→</span></a></div><span className="card-number">{number}</span></article>; }
